...
参与了湖南农信 财务报表系统后期的一些开发、维护工作

2016年底作为4人开发小组的组长，主导了北京中关村银行 企业报表系统的 部署开发上线全流程，
从前期项目启动，环境搭建，需求讨论，到项目开发、流程调试，
上线前项目文档验收 自己都参与了其中

2018年 参与了一个CRM项目，
作为PAAS平台的元数据服务小组成员，主要负责元数据服务的临时功能维护 和
新业务功能的扩展，包括对公司其他业务线和公司的合作伙伴提供rest接口，

另外也有配合中间件小组最元数据服务进行一些技术栈升级， 
比如替换ID生成器，kestrel转rabbit MQ

2019年全年参与了一个机票代理平台的4.0版本迭代，项目与2020年元旦成功上线     
自己作为项目核心开发成员，主要负责订单服务，政策服务，基础数据服务的功能开发


希望接下来继续从事Java服务端开发的工作

## 项目中遇到的难点
#### 1. 多人同时维护基础数据时，post请求 需要保证幂等性，同时记录数据变动日志
...
2. 继承 AbstractRoutingDataSource 重写 determineCurrentLookupKey 方法放回当前线程数据源 
...
JDK7 线程扩容时的搬运结点操作 为了提高效率采用的头插法 导致线程之前看到了错误的next指向，导致成环，       
...
在Java多线程情况下，volatile修饰的变量能保证线程之后的可见性、有序性，不能保证原子性，

不能保证线程安全，比如 i++

通过CAS机制实现线程安全，在Java中Unsafe对CAS进行了封装

#### synchronized 是公平锁吗，实现锁的原理，获取对象的monitor 属于对象什么信息
...
2. 将原先table数组＋单向链表的数据结构，变更为table数组＋单向链表＋红黑树的结构， 避免链表过长

##### concurrentHashMap 怎么实现线程安全，分段锁是jdk几提出的
JDK1.8提出分段锁，put时, synchronized 将Node链表的头节点作为锁，

在初始化数组时使用了 volatile + CAS操作 决定到底是哪个线程有资格进行初始化，其他线程均只能等待
...
3. 整合常用依赖，提供的POM可以简化Maven的配置。当我们引入核心依赖时，SpringBoot会自引入其他依赖
